function scatter_draw() {
	...
	var Xaxis = modules['scatter']['variables']['Xaxis'],
		...
		Size = modules['scatter']['variables']['Size'],
		Shape = modules['scatter']['variables']['Shape'],
		mdimension = flight.dimension(function(d){ return [+d[Xaxis], +d[Yaxis], d[Group], d[Shape]];}),
		mgroup = (Size == 'none') ?  mdimension.group() : mdimension.group().reduceSum(function(d) { return   +d[Size]; });
...
function scatter_update() {
	...
	data.forEach(function(d,i){
		if (d.value > 0) {
			...
			scatter_data[d.key[2]].values.push({x:d.key[0], y:d.key[1], size:d.value, shape:shapes(d.key[3])});
		}
	});
